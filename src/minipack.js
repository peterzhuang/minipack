...
function bundle(graph) {
  ...
  // return result;
  writeFileRecursive(
    './bundle/production-bundle.js',
    UglifyJS.minify(result).code,
    err => {
      if (err) {
        console.log(`error generating production-bundle.js ${err}`);
      }
      console.log(`production-bundle.js successfully created.`);
    },
  );
}

function writeFileRecursive(filename, content, callback) {
  ...
  // -- write file
  fs.writeFile(root + filepath, content, callback);
}
